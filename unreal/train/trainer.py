...
from unreal.environment.gym_environment import GymEnvironment
# from environment.ale_environment import AleEnviroment
from unreal.model.model import UnrealModel
from unreal.train.experience import Experience, ExperienceFrame
...
class Trainer(object):
  def __init__(self,
    ...
    self.action_size = GymEnvironment.get_action_size()
    # self.action_size = AleEnviroment.get_action_size()
    ...
    #
    # self.apply_gradients = grad_applier.minimize(tf.square(self.local_network.total_loss +
    #                                                    global_network.get_vars()+
    #                                                    self.local_network.get_vars()))
    #
    self.sync = self.local_network.sync_from(global_network)
    self.environment = GymEnvironment.create_environment()
    # self.environment = AleEnviroment.create_environment()
  ...
  def minimize_local_2(self, loss, global_var_list, local_var_list):
    """
    ...
    with tf.device("/cpu:0"):
      ...
      return self.grad_applier.apply_gradients(zip(global_var_list, var_refs))
