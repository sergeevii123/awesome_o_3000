...
from async_dqn.asyncrl.environment import GymWrapperFactory
from async_dqn.asyncrl.agent import QlearningAgent, AgentSummary
import time
import os
import gym
...
tf.app.flags.DEFINE_float("gamma", 0.999, "Gamma discount factor")
...
tf.app.flags.DEFINE_string("logdir", './async_dqn/logs/', "Path to the directory used for checkpoints and loggings")
...
tf.app.flags.DEFINE_integer("eval_iter", 200, "Number of evaluation episodes")
# Optimizer
...
def evaluate():
    ...
    with tf.Session() as sess:
        ...
        curr_reward = []
        for fi in range(FLAGS.eval_iter):
            s = envwrap.reset()
            terminal = False
            total_reward = 0
            while not terminal:
                ...
                total_reward += r
                # envwrap.render()
            curr_reward.append(total_reward)
            print (fi, np.mean(curr_reward[-100:]))
        # envwrap.env.monitor.close()
        print('Evaluation finished.')
        print('Average reward per episode: %0.4f' % np.mean(curr_reward[-100:]))
