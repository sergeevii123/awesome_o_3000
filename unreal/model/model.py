...
class UnrealModel(object):
  ...
  def _base_conv_layers(self, state_input, reuse=False):
    with tf.variable_scope("base_conv", reuse=reuse) as scope:
      conv1 = slim.conv2d(inputs=state_input, num_outputs=16,
      ...
      # conv3 = slim.conv2d(inputs=conv2, num_outputs=64,
      #                     kernel_size=[4, 4], stride=[2, 2], padding='VALID')
      # conv4 = slim.conv2d(inputs=conv3, num_outputs=64,
      #                     kernel_size=[2, 2], stride=[1, 1], padding='VALID')
      # conv3 = slim.conv2d(inputs=conv2, num_outputs=32,
      #                     kernel_size=[2, 2], stride=[1, 1], padding='VALID')
      return conv2
  ...
  def _create_rp_network(self):
    ...
    rp_conv_output_rehaped = tf.reshape(rp_conv_output, [1,7776])
    
    with tf.variable_scope("rp_fc") as scope:
      # Weights
      W_fc1, b_fc1 = self._fc_variable([7776, 3], "rp_fc1")
